"""Thermodynamic conversion and Fortran bridge utilities."""
...
_LOG_T_POWER_SENTINEL = 99.0


def _resolve_deprecated_keyword(
    value: Any,
    kwargs: Dict[str, Any],
    old_name: str,
    new_name: str,
) -> Any:
    """Resolve one deprecated keyword alias."""
    if old_name in kwargs:
        if value is not None:
            raise TypeError(f"received both '{new_name}' and deprecated '{old_name}'")
        warnings.warn(
...
def G2HSCp_single(
    gibbs_energy: List[float] | None = None,
    **kwargs: Any,
) -> Tuple[float, float, np.ndarray]:
    """
    Convert one temperature interval's Gibbs energy coefficients.

    Input
    -----
    gibbs_energy : A list of 14 floats
        [a, b, c, d, e, f, c1, p1, c2, p2, c3, p3, c4, p4].
        a-f are coefficients of the predefined function, while ci and pi
        are coefficients and powers of custom functions.
        G = a + b*T + c*T*ln(T) + d*T**2 + e*T**3 + f*T**(-1)
            + c1*T**p1 + c2*T**p2 + c3*T**p3 + c4*T**p4

    Output
    ------
    H298 : Enthalpy of formation at 298.15 K
    S298 : Vibration entropy at 298.15 K
    Cp(T): A list of 12 floats
        [a, b, c, d, c1, p1, c2, p2, c3, p3, c4, p4].
        Cp(T) = a + b*T + c*T**(-2) + d*T**2
            + c1*T**p1 + c2*T**p2 + c3*T**p3 + c4*T**p4

    """
    gibbs_energy = _resolve_deprecated_keyword(
        gibbs_energy, kwargs, "GibbsEnegy", "gibbs_energy"
    )

    # assert isinstance(Gibbs,list), "Error: input must be a list with length 14"
    assert len(gibbs_energy) == 14, "Error: input must have length of 14"

    a, b, c, d, e, f, c1, p1, c2, p2, c3, p3, c4, p4 = gibbs_energy
    ...
    for i, param in enumerate(custom):
        ...
        if _is_log_t_power(power):
            S298 = S298 - coeff / T
            H298 = H298 + coeff * (np.log(T) - 1)
            j = int(2 * i + 4)
            Cp[j] = coeff
            Cp[j + 1] = _LOG_T_POWER_SENTINEL
            continue
...
def HSCp2G(
    H298: float, S298: float, Cp: Union[List[List[float]], np.ndarray]
) -> np.ndarray:
    """
    Convert H298, S298, and Cp(T) to Gibbs energies.
    ...
    Output
    ------
    Gibbs energies : A list of 16 floats
    ...
    """
    Cp = np.asarray(Cp)
    n, n_columns = Cp.shape  # number of temperature intervals

    assert n_columns == 14, "Error: input must have length of 14"
    ...
    for i, cp in enumerate(Cp):
        ...
        for k, param in enumerate(custom):
            ...
            if _is_log_t_power(power):
                log_t_coefficients += coeff
                j = int(2 * k + 8)
                GibbsEnergies[i, j] = coeff
                GibbsEnergies[i, j + 1] = _LOG_T_POWER_SENTINEL
                continue
        ...
        if i != 0:
            ...
            H298 = (
                H298
                + IntCp_Hprev
                - _IntCp_H(cp_coefficients, Tmin)
                + _IntCp_H(cp_coefficients, 298.15)
            )
            S298 = (
                S298
                + IntCp_Sprev
                - _IntCp_S(cp_coefficients, Tmin)
                + _IntCp_S(cp_coefficients, 298.15)
            )

        Tmax = GibbsEnergies[i, 1]

        GibbsEnergies[i, 2] = (
            GibbsEnergies[i, 2] + H298 - _IntCp_H(cp_coefficients, 298.15)
        ) + log_t_coefficients  # a term
        GibbsEnergies[i, 3] = (
            GibbsEnergies[i, 3] - S298 + a + _IntCp_S(cp_coefficients, 298.15)
        )  # b*T term
        ...
        IntCp_Hprev = _IntCp_H(cp_coefficients, Tmax) - _IntCp_H(
            cp_coefficients, 298.15
        )
        IntCp_Sprev = _IntCp_S(cp_coefficients, Tmax) - _IntCp_S(
            cp_coefficients, 298.15
        )

    # Ensure that the final temperature is 6000 K
    if GibbsEnergies[-1, 1] <= 6000:
        GibbsEnergies[-1, 1] = 6000

    return GibbsEnergies


def G2HSCp(
    gibbs_energies: Union[List[List[float]], np.ndarray] | None = None,
    **kwargs: Any,
) -> Tuple[float, float, np.ndarray]:
    """
    Convert Gibbs energy coefficients over temperature intervals.
    ...
    G(T) = a + b*T + c*T*ln(T) + d*T**2 + e*T**3 + f*T**(-1)
        + c1*T**p1 + c2*T**p2 + c3*T**p3 + c4*T**p4
    ...
    Parameters
    ----------
    gibbs_energies : Union[List[List[float]], np.ndarray]
    ...
    """
    gibbs_energies = _resolve_deprecated_keyword(
        gibbs_energies, kwargs, "GibbsEnegies", "gibbs_energies"
    )

    # assert isinstance(Gibbs,list), "Error: input must be a list with length 14"
    gibbs_energies = np.asarray(gibbs_energies)
    n, n_columns = gibbs_energies.shape

    assert n_columns == 16, "Error: input must have length of 16"

    # Sort Gibbs energy by Tmin
    Tmins = gibbs_energies[:, 0]
    idx = np.argsort(Tmins)
    gibbs_energies = gibbs_energies[idx, :]

    Cp = np.zeros((n, 14))
    Cp[:, 0] = gibbs_energies[:, 0]
    Cp[:, 1] = gibbs_energies[:, 1]
    for i in range(n):
        H, S, cp = G2HSCp_single(gibbs_energies[i, 2:])
        if i == 0:
            H298 = H
            S298 = S
        Cp[i, 2:] = cp
...
def NeumanKoppHSCp(
    dataset: Dict[str, Dict[str, Union[float, np.ndarray]]],
    names: List[str],
    multipliers: Union[List[float], np.ndarray],
) -> Dict[str, Union[float, np.ndarray]]:
    """
    Calculate mixture thermodynamic properties using the Neumann-Kopp rule.
    ...
    -----
    dataset : dict
            A dictionary containing the thermodynamic data for functions/species.
            Each entry should have 'H298', 'S298', and 'Cp' data.
    names : list of str
            A list of function/species names to be included in the calculation.
            These names must be keys in the `dataset` dictionary.
    multipliers : list or array of float
            The corresponding multipliers for each species in `names`.
    ...
    res : dict
            ...
            - 'Cp': A numpy array representing heat capacity over temperature
            ...
            for k, p in enumerate(custom_power):
                ...
                if lmultiplier == 1:
                    ...
                    Cp_new[int(6 + 2 * k)] = (
                        ...
                        * Cp_temp[imultiplier, ci_index[ipower]]
                    )
                else:
                    Cp_new[int(7 + 2 * k)] = p
                    Cp_new[int(6 + 2 * k)] = np.matmul(
                        multipliers[imultiplier], Cp_temp[imultiplier, ci_index[ipower]]
                    )
        if i == 0:
    ...
    res = {"H298": H298_new, "S298": S298_new, "Cp": Cps_new}
    return res
...
def _IntCp_H(cp: np.ndarray, T: float) -> float:
    """
    Calculate the Cp integral for enthalpy change.

    This function analytically integrates the heat capacity polynomial for a
    given temperature T, which is used to compute the enthalpy change.
    ...
    for param in custom:
        coeff, power = param
        # Keep any nonzero coefficient: high powers can turn tiny coefficients
        # into finite thermodynamic contributions.
        if coeff == 0.0:
            continue
...
def _IntCp_S(cp: np.ndarray, T: float) -> float:
    """
    Calculate the Cp/T integral for entropy change.

    This function analytically integrates the heat capacity polynomial divided
    by temperature for a given temperature T.
...
def _is_log_t_power(power: float) -> bool:
    """Return whether a custom term uses the FactSage-style ln(T) sentinel."""
    return abs(power - _LOG_T_POWER_SENTINEL) < 1e-8
...
def _dict2np(
    ...
    """
    Convert a dictionary to a NumPy array with consistent column lengths.
...
def _pyvar2fvar(var: object) -> None:
    """
    Pass Python variables to Fortran modules.
    ...
    fort.str3d(
        "cConstituentNameSUBCS", np.asarray(var.cConstituentNameSUBCS, np.dtype("c"))
    )

    # Appending 0th index to 1st index
    fort.moduleparsecs.nparamphasecs = np.append(
        [0], var.nParamPhaseCS
    )  # CheckSystemExcess,Compthermodata
    fort.moduleparsecs.nspeciesphasecs = np.append(
        [0], var.nSpeciesPhaseCS
    )  # Checksystem,CheckSystemExcess,Compthermodata
    fort.moduleparsecs.nmagparamphasecs = np.append(
        [0], var.nMagParamPhaseCS
    )  # CheckSystemExcess,Compthermodata
