"""Parse Gibbs-energy entries from ChemSage data blocks."""
...
from .utils import G2HSCp, HSCp2G, NeumanKoppHSCp


def ParseCSDataBlockGibbs(i, j):
    """Parse Gibbs-energy data for one species."""
    ...
    # Record particles per constituent formula mass (stoichio of constituents).
    if "):" in var.cSpeciesNameCS[j - 1]:
        cDummy = var.cSpeciesNameCS[j - 1]
        var.iParticlesPerMoleCS[j - 1] = re.search(
            r"\d+", cDummy[cDummy.find(":") :]
        ).group()

    # Check for a dummy species. The header describes "dummy species".
    ...
    # ChemSage data-files store the stoichiometry coefficients as real
    # variables, which may be less than one when there is more than one
    # particle/mole. Convert stoichiometry to an integer to minimize numerical
    # error while still recording the number of particles per mole.
    dTemp = float(var.iParticlesPerMoleCS[j - 1])
    var.dStoichSpeciesCS[j - 1, : var.nElementsCS] = var.dTempVec * dTemp

    # Loop_GibbsEquations
    for _ in range(var.nGibbsEqSpecies[j - 1]):
        # Entry 5: read upper temperature limit and Gibbs energy coefficients
        # for this species.
        ...
        var.dGibbsCoeffSpeciesTemp[:7, var.iCounterGibbsEqn] = np.asarray(
            var.DataBase[:7], dtype=float
        )
        del var.DataBase[:7]
        ...
        if iGibbsEqType in [4, 16, 5, 6]:
            # Gibbs energy equations designated type 4 and 16 have an
            # additional
            # line, but type 1 does not.
            # Read the number of additional standard Gibbs energy terms.
            n_extra_terms = int(var.DataBase.pop(0))
        if n_extra_terms == 0:
            pass
        elif n_extra_terms == 1:
            var.dGibbsCoeffSpeciesTemp[7:9, var.iCounterGibbsEqn] = np.asarray(
                var.DataBase[:2], dtype=float
            )
            del var.DataBase[:2]
        elif n_extra_terms == 2:
            var.dGibbsCoeffSpeciesTemp[7:11, var.iCounterGibbsEqn] = np.asarray(
                var.DataBase[:4], dtype=float
            )
            del var.DataBase[:4]
        elif n_extra_terms == 3:
            var.dGibbsCoeffSpeciesTemp[7:13, var.iCounterGibbsEqn] = np.asarray(
    ...
    if iGibbsEqType in [3, 6]:
        ...
        del var.DataBase[:11]
    # Assign an arbitrarily large positive value for dummy species Gibbs
    # energy equations.
...
def ChangeBlockGibbs():
    """Convert FactSage 8+ Gibbs blocks to the legacy parser layout."""
    res = []
