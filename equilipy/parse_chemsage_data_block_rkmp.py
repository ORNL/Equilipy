"""Parse RKMP solution model data blocks."""
...
def ParseCSDataBlockRKMP(i):
    """Parse RKMP excess parameters for one solution phase."""
    ...
    if "RKMPM" in var.cSolnPhaseTypeCS[i - 1]:
        ...
        while True:
            ...
            if var.iMagneticParamCS[var.nMagParamCS, 0] == 0:
                ...
                var.iMagneticParamCS[var.nMagParamCS, 1:4] = np.asarray(
                    var.DataBase[:3], dtype=int
                )
                del var.DataBase[:3]
                ...
                for j in range(1, k + 1):
                    # Record mixing ID's:
                    var.iMagneticParamCS[var.nMagParamCS, :3] = var.iMagneticParamCS[
                        var.nMagParamCS - j + 1, :3
                    ]
                ...
                # Ternary mixing
                var.iMagneticParamCS[var.nMagParamCS, 1:5] = np.asarray(
                    var.DataBase[:4], dtype=int
                )
                del var.DataBase[:4]
                ...
                for j in range(1, k + 1):
                    # Record mixing ID's:
                    var.iMagneticParamCS[var.nMagParamCS, :5] = var.iMagneticParamCS[
                        var.nMagParamCS - j + 1, :5
                    ]

                    # Set the exponent:
                    var.iMagneticParamCS[var.nMagParamCS, 4] = var.iMagneticParamCS[
                        var.nMagParamCS, j
                    ]
                    var.dMagneticParamCS[var.nMagParamCS, :2] = np.asarray(
                        var.DataBase[:2], dtype=float
                ...
                # Quaternary mixing
                var.iMagneticParamCS[var.nMagParamCS, 1:6] = np.asarray(
                    var.DataBase[:5], dtype=int
                )
                del var.DataBase[:5]
                ...
                for j in range(1, k + 1):
                    # Record mixing ID's:
                    var.iMagneticParamCS[var.nMagParamCS, :6] = var.iMagneticParamCS[
                        var.nMagParamCS - j + 1, :6
                    ]
    ...
    while True:
        ...
        if var.iRegularParamCS[var.nParamCS, 0] == 0:
            ...
            # Binary mixing
            var.iRegularParamCS[var.nParamCS, 1:4] = np.asarray(
                var.DataBase[:3], dtype=int
            )
            del var.DataBase[:3]
            # Store the number of equations to temporary memory
            ...
            for j in range(1, k + 1):
                # Record mixing ID's
                var.iRegularParamCS[var.nParamCS, :4] = var.iRegularParamCS[
                    var.nParamCS - j + 1, :4
                ]
            ...
            # Ternary mixing
            var.iRegularParamCS[var.nParamCS, 1:5] = np.asarray(
                var.DataBase[:4], dtype=int
            )
            del var.DataBase[:4]
            ...
            for j in range(1, k + 1):
                # Record mixing ID's
                var.iRegularParamCS[var.nParamCS, :5] = var.iRegularParamCS[
                    var.nParamCS - j + 1, :5
                ]
            ...
            # Quaternary mixing
            var.iRegularParamCS[var.nParamCS, 1:6] = np.asarray(
                var.DataBase[:5], dtype=int
            )
            del var.DataBase[:5]
            ...
            for j in range(1, k + 1):
                # Record mixing ID's
                var.iRegularParamCS[var.nParamCS, :6] = var.iRegularParamCS[
                    var.nParamCS - j + 1, :6
                ]
