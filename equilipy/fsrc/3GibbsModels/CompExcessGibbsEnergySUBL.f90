...
!
    ...
    real(8) :: dTemp, dFirstParam, dSecondParam, dFirstParam2, dSecondParam2, dThirdParam
    real(8) :: dPreFactorG, dPreFactorH, dPreFactorS, dPreFactorCp
...
!
                ! Update the reference molar Gibbs energy, Enthalpy, Entropy, Heatcapacity:
                dChemicalPotential(i) = dChemicalPotential(i) + dTemp * dMolFraction(j) * dStdGibbsEnergy(j)
                dPartialEnthalpy(i)   = dPartialEnthalpy(i) + dTemp * dMolFraction(j) * dStdEnthalpy(j)
                dPartialEntropy(i)    = dPartialEntropy(i) + dTemp * dMolFraction(j) * dStdEntropy(j)
                dPartialHeatCapacity(i) = dPartialHeatCapacity(i) + dTemp * dMolFraction(j) * dStdHeatCapacity(j)

            end do LOOP_Ideal_Components
!
            ! Add ideal mixing contribution (Update for G, and S, unitless):
            do s = 1, nSublattice
                c = iConstituentSublattice(iChargedPhaseID,s,m)
                dChemicalPotential(i) = dChemicalPotential(i) + dStoichSublattice(iChargedPhaseID,s) &
                    * DLOG(dSiteFraction(iChargedPhaseID,s,c))
                    
                dPartialEntropy(i)    = dPartialEntropy(i) - dStoichSublattice(iChargedPhaseID,s) &
                * DLOG(dSiteFraction(iChargedPhaseID,s,c))
            end do
!
...
!
            ...
            if ((iSUBLParamData(l,1) == 1) .AND. (iSUBLParamData(l,3) == 2)) then
                ...
                do k = 2, n + 1
                    ...
                    dPreFactorG = dPreFactorG * dSiteFraction(iChargedPhaseID,s,c)
                    dPreFactorH = dPreFactorH * dSiteFraction(iChargedPhaseID,s,c)
                    dPreFactorS = dPreFactorS * dSiteFraction(iChargedPhaseID,s,c)
                    dPreFactorCp = dPreFactorCp * dSiteFraction(iChargedPhaseID,s,c)
                ...
                dPreFactorG = dPreFactorG * dExcessGibbsParam(l) * (dFirstParam - dSecondParam)**(iExponent)
                dPreFactorH = dPreFactorH * dExcessHParam(l) * (dFirstParam - dSecondParam)**(iExponent)
                dPreFactorS = dPreFactorS * dExcessSParam(l) * (dFirstParam - dSecondParam)**(iExponent)
                dPreFactorCp = dPreFactorCp * dExcessCpParam(l) * (dFirstParam - dSecondParam)**(iExponent)
                ...
                ! Multiply prefactor term by excess Gibbs energy parameter:
                iExponent = 0
                dPreFactorG = dPreFactorG * dExcessGibbsParam(l)
                dPreFactorH = dPreFactorH * dExcessHParam(l)
                dPreFactorS = dPreFactorS * dExcessSParam(l)
                dPreFactorCp = dPreFactorCp * dExcessCpParam(l)

                dPreFactorG = dPreFactorG * (dFirstParam + (1D0 - dFirstParam - dSecondParam - dThirdParam) / 3D0)
                dPreFactorH = dPreFactorH * (dFirstParam + (1D0 - dFirstParam - dSecondParam - dThirdParam) / 3D0)
                dPreFactorS = dPreFactorS * (dFirstParam + (1D0 - dFirstParam - dSecondParam - dThirdParam) / 3D0)
                dPreFactorCp = dPreFactorCp * (dFirstParam + (1D0 - dFirstParam - dSecondParam - dThirdParam) / 3D0)
...
!
                ...
                dPartialExcessGibbs(i) = dPartialExcessGibbs(i) + dPreFactorG * dTemp
                dPartialEnthalpyXS(i)  = dPartialEnthalpyXS(i) + dPreFactorH * dTemp
                dPartialEntropyXS(i)   = dPartialEntropyXS(i) + dPreFactorS * dTemp
                dPartialHeatCapacityXS(i) = dPartialHeatCapacityXS(i) + dPreFactorCp * dTemp
!
