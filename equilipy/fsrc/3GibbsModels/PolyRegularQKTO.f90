...
!
    ...
    !   11/11/2020      S.Y.   Kwon         Removed dPartialGParam, it is assigned in ModuleGEMSolver
    !
    ! Purpose
    ! =======
    !
...
!
    ...
    dHParam        = dExcessHParam(iParam)
    dSParam        = dExcessSParam(iParam)
    dCpParam        = dExcessCpParam(iParam)
    dPartialGParam = 0D0
!
...
!
    ...
    do i = 1, iRegularParam(iParam,1)
        ...
        dHParam = dHParam * (y(i) ** iRegularParam(iParam,k))
        dSParam = dSParam * (y(i) ** iRegularParam(iParam,k))
        dCpParam = dCpParam * (y(i) ** iRegularParam(iParam,k))
    end do
!
    ...
    do i = 1, iRegularParam(iParam,1)
        ...
        dPartialHParam(i) = dExcessHParam(iParam) * (DFLOAT(k) * y(i)**(k - 1) + (1D0 - DFLOAT(zT)) * y(i)**k)
        dPartialSParam(i) = dExcessSParam(iParam) * (DFLOAT(k) * y(i)**(k - 1) + (1D0 - DFLOAT(zT)) * y(i)**k)
        dPartialCpParam(i) = dExcessCpParam(iParam) * (DFLOAT(k) * y(i)**(k - 1) + (1D0 - DFLOAT(zT)) * y(i)**k)
!
        ...
        do j = 1, iRegularParam(iParam,1)
            ...
            dPartialHParam(i) = dPartialHParam(i) * y(j)**m
            dPartialSParam(i) = dPartialSParam(i) * y(j)**m
            dPartialCpParam(i) = dPartialCpParam(i) * y(j)**m
        end do
!
    end do
!
