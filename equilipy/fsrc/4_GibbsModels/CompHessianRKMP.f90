!> \brief Compute analytical RKMP chemical-potential Jacobians.
!!
!! \details This diagnostic/minimizer routine evaluates one-sublattice RKMP
...
subroutine CompHessianRKMP(iSolnIndex, nSpeciesDim, dSpeciesChemicalPotential, dSpeciesHessian, &
    nSpeciesOut, iInfo)
!
    !-------------------------------------------------------------------------------------------------------------
    !
    !> \file    CompHessianRKMP.f90
    !> \brief   Compute analytical RKMP chemical-potential Jacobians.
    !> \author  S.Y. Kwon
    !> \date    Jun. 28, 2026
    ...
    !! \f$J_{ij}=\partial \mu_i/\partial x_j\f$ for a nonmagnetic RKMP
    !! one-sublattice phase.  The excess part is built by differentiating the
    !! scalar Muggianu polynomial terms and projecting scalar curvature to
    ...
    !
    ! cSolnPhaseType             Solution model type.  RKMP is accepted.
    ! dMolFraction               Current phase-local species mole fractions.
    ! dExcessGibbsParam          Dimensionless RKMP excess parameters.
    ! iRegularParam              RKMP binary/ternary/quaternary parameter topology.
    !
    !
    ...
    ! =============================
    !
    ! AccumulateRKMPParameterScalar  Adds scalar excess value, gradient, and Hessian for one RKMP parameter.
    ...
    ! - This routine supports the nonmagnetic RKMP excess curvature.  RKMPM returns
    ...
    if (TRIM(cSolnPhaseType(iSolnIndex)) == 'RKMPM') then
        iInfo = 4
        return
    end if
    if (TRIM(cSolnPhaseType(iSolnIndex)) /= 'RKMP') then
        iInfo = 1
        return
    end if
    ...
    do iParam = nParamPhase(iSolnIndex-1) + 1, nParamPhase(iSolnIndex)
        call AccumulateRKMPParameterScalar(iFirst, nSpeciesOut, iParam, dScalarGradient, &
...
contains
    ...
    subroutine AccumulateRKMPParameterScalar(iFirstIn, nLocal, iParamIn, dGradientOut, &
        ...
        integer :: i, iMixType, iExponent, iSpeciesA, iSpeciesB, iSpeciesC, iSpeciesD, iTernarySpecies
        real(8) :: dTermValue
        ...
        select case (iMixType)
            case (2)
                iSpeciesA = iRegularParam(iParamIn,2)
                iSpeciesB = iRegularParam(iParamIn,3)
                iExponent = iRegularParam(iParamIn,4)
                if ((iSpeciesA < 1).OR.(iSpeciesA > nLocal).OR.(iSpeciesB < 1).OR.(iSpeciesB > nLocal)) then
                    iInfoOut = 2
                    return
                end if

                dFactorGradient = 0D0
                dFactorGradient(iSpeciesA) = 1D0
                call MultiplyLinearPower(nLocal, dMolFraction(iFirstIn+iSpeciesA-1), dFactorGradient, 1, &
                    dTermValue, dTermGradient, dTermHessian)

                dFactorGradient = 0D0
                dFactorGradient(iSpeciesB) = 1D0
                call MultiplyLinearPower(nLocal, dMolFraction(iFirstIn+iSpeciesB-1), dFactorGradient, 1, &
                    dTermValue, dTermGradient, dTermHessian)

                dFactor = dMolFraction(iFirstIn+iSpeciesA-1) - dMolFraction(iFirstIn+iSpeciesB-1)
                dFactorGradient = 0D0
                dFactorGradient(iSpeciesA) = 1D0
                dFactorGradient(iSpeciesB) = -1D0
                call MultiplyLinearPower(nLocal, dFactor, dFactorGradient, iExponent, &
                    dTermValue, dTermGradient, dTermHessian)

            case (3)
                iSpeciesA = iRegularParam(iParamIn,2)
                iSpeciesB = iRegularParam(iParamIn,3)
                iSpeciesC = iRegularParam(iParamIn,4)
                iTernarySpecies = iRegularParam(iParamIn,5)
                if ((iSpeciesA < 1).OR.(iSpeciesA > nLocal).OR.(iSpeciesB < 1).OR.(iSpeciesB > nLocal).OR.&
                    (iSpeciesC < 1).OR.(iSpeciesC > nLocal).OR.&
                    (iTernarySpecies < 1).OR.(iTernarySpecies > nLocal)) then
                    iInfoOut = 2
                    return
                end if

                do i = 2, 4
                    iSpeciesA = iRegularParam(iParamIn,i)
                    dFactorGradient = 0D0
                    dFactorGradient(iSpeciesA) = 1D0
                    call MultiplyLinearPower(nLocal, dMolFraction(iFirstIn+iSpeciesA-1), dFactorGradient, 1, &
                        dTermValue, dTermGradient, dTermHessian)
                ...
                do i = 2, 4
                    iSpeciesA = iRegularParam(iParamIn,i)
                    dFactor = dFactor - dMolFraction(iFirstIn+iSpeciesA-1) / 3D0
                    dFactorGradient(iSpeciesA) = dFactorGradient(iSpeciesA) - 1D0 / 3D0
                end do
                dFactor = dFactor + dMolFraction(iFirstIn+iTernarySpecies-1)
                dFactorGradient(iTernarySpecies) = dFactorGradient(iTernarySpecies) + 1D0
                call MultiplyLinearPower(nLocal, dFactor, dFactorGradient, 1, &
                    dTermValue, dTermGradient, dTermHessian)
            ...
            case (4)
                ...
                iSpeciesB = iRegularParam(iParamIn,3)
                iSpeciesC = iRegularParam(iParamIn,4)
                iSpeciesD = iRegularParam(iParamIn,5)
                if ((iSpeciesA < 1).OR.(iSpeciesA > nLocal).OR.(iSpeciesB < 1).OR.(iSpeciesB > nLocal).OR.&
                    (iSpeciesC < 1).OR.(iSpeciesC > nLocal).OR.(iSpeciesD < 1).OR.(iSpeciesD > nLocal)) then
                    iInfoOut = 2
                    return
                end if
    ...
    end subroutine AccumulateRKMPParameterScalar
...
end subroutine CompHessianRKMP
