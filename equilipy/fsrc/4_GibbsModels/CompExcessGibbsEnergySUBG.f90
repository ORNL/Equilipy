...
!
        ...
        if (dSum == 0) then
            ...
            dEntropy = dEntropy + DLOG(DMAX1(dMolFraction(l), 1D-75) / dSum)
        end if
!
        ...
        dPartialEntropy(l)      = dStdEntropy(l)     - dEntropy
...
!
        ...
        if (x == y) then
            ...
            ! SUBQ mixed quadruplets contribute half to each asymmetric side.
...
!
        ...
        ! Reciprocal terms
            ...
            dPartialExcessGibbs(iBlock) = dPartialExcessGibbs(iBlock) + dExcessGibbsParam(abxy)
            dPartialEnthalpyXS(iBlock) = dPartialEnthalpyXS(iBlock) + dExcessHParam(abxy)
            dPartialEntropyXS(iBlock) = dPartialEntropyXS(iBlock) + dExcessSParam(abxy)
            dPartialHeatCapacityXS(iBlock) = dPartialHeatCapacityXS(iBlock) + dExcessCpParam(abxy)
            cycle LOOP_Param
        else
            INFOThermo = 42
        end if
...
!
            ...
            if (cRegularParam(abxy) == 'G') then
                ...
                if (lAsymmetric1(i) .AND. lAsymmetric1(j)) then
                    ...
                    dDHex = dDHex + dChiFactor * dHex * p / dChi1 / dChiDen
                    dDSex = dDSex + dChiFactor * dSex * p / dChi1 / dChiDen
                    dDCpex = dDCpex + dChiFactor * dCpex * p / dChi1 / dChiDen
                end if
                if (lAsymmetric2(i) .AND. lAsymmetric2(j)) then
                    dDgex = dDgex + dChiFactor * dGex * q / dChi2 / dChiDen
                    dDHex = dDHex + dChiFactor * dHex * q / dChi2 / dChiDen
                    dDSex = dDSex + dChiFactor * dSex * q / dChi2 / dChiDen
                    dDCpex = dDCpex + dChiFactor * dCpex * q / dChi2 / dChiDen
                end if
                if ((lAsymmetric1(i) .OR. lAsymmetric2(i)) .AND. (lAsymmetric1(j) .OR. lAsymmetric2(j))) then
                    dDgex = dDgex + dChiFactor * dDgexBase
                    dDHex = dDHex + dChiFactor * dDHexBase
                    dDSex = dDSex + dChiFactor * dDSexBase
                    dDCpex = dDCpex + dChiFactor * dDCpexBase
                end if
!
                ...
                do ii = 1, nSublatticeElements(iSPI,1)
                    ...
                    if (lAsymmetric2(ii)) then
                        if (ii == i .AND. x == k) then
                            ...
                            dDHex = dDHex + dDHexBase / 4 + dHex * q / (4 * dXi2)
                            dDSex = dDSex + dDSexBase / 4 + dSex * q / (4 * dXi2)
                            dDCpex = dDCpex + dDCpexBase / 4 + dCpex * q / (4 * dXi2)
                        end if
                        if (ii == i .AND. x == l) then
                            dDgex = dDgex + dDgexBase / 4 + dGex * q / (4 * dXi2)
                            dDHex = dDHex + dDHexBase / 4 + dHex * q / (4 * dXi2)
