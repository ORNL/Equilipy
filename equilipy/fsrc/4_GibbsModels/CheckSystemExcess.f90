...
!
                ! Count the number of constituents on each sublattice and
                ! build a compact original-ID -> local-ID map.  Some DAT/CEF
                ! phases use constituent IDs that are not 1..nConstituent for
                ! a given sublattice (for example a pure Va sublattice may
                ! reference constituent ID 3), so scan the full constituent
                ! index dimension rather than only nConstituentSublatticeCS.
                do s = 1, nSublatticePhaseCS(k)
!
                    ! Loop through constituents on sublattice s:
                    j = 0
                    do c = 1, SIZE(iConstituentPass, DIM=3)
                        ! Store the correct constituent name:
                        if (iConstituentPass(k,s,c) /= 0) then
                            j = j + 1
                            nConstituentSublattice(nCountSublattice,s) = j
                            iConstituentPass(k,s,c) = j
...
!
                ...
                LOOP_findPassed: do c = 1, SIZE(iConstituentSublatticeCS, DIM=3)
                    ...
                    j = j + 1
                    do s = 1, nSublatticePhaseCS(k)
                        m = iConstituentSublatticeCS(nCountSublatticeCS,s,c)
                        iConstituentSublattice(nCountSublattice,s,j) = iConstituentPass(k,s,m)
                    end do
