...
!
    ! A one-species solution still has a valid phase Gibbs scalar.  This path
    ! is common after active-component screening reduces an oxide solution to a
    ! unary pseudo-component.
    lModelSpecificUnary = (cSolnPhaseType(iSolnIndex) == 'SUBL').OR. &
        (cSolnPhaseType(iSolnIndex) == 'SUBLM').OR. &
        (cSolnPhaseType(iSolnIndex) == 'SUBOM').OR. &
        (cSolnPhaseType(iSolnIndex) == 'SUBG').OR. &
        (cSolnPhaseType(iSolnIndex) == 'SUBQ')

    if ((iLast == iFirst).AND.(.NOT.lModelSpecificUnary)) then
        ...
        dGibbsSolnPhase(iSolnIndex) = dChemicalPotential(iFirst) * dMolesSpecies(iFirst)
        return
    end if
...
!
            ...
            do i = iFirst, iLast
                if(dMolFraction(i)< 1D-75) then
                    ...
                    dChemicalPotential(i)       = dStdGibbsEnergy(i) + DLOG(DMAX1(dMolFraction(i), 1D-75)) + &
                                                  dPartialExcessGibbs(i) + dMagGibbsEnergy(i)
                    dPartialEnthalpy(i)         = dStdEnthalpy(i) + dPartialEnthalpyXS(i) + dMagEnthalpy(i)
                    dPartialEntropy(i)          = dStdEntropy(i)  - DLOG(DMAX1(dMolFraction(i), 1D-75)) + &
                                                  dPartialEntropyXS(i) + dMagEntropy(i)
