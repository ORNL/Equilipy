...
!
    ...
    ! Exit only when this phase has a single endmember.
...
!
        ...
        IF_Tau: if (tau > 1D0) then
            ...
            ftp = (237D0/(70D0*p*dTemperature)&
                - (474D0/(497D0*Tcritical))*invpmone*(dTempA + 56D0*dTempB/15D0 + 91D0*dTempC/20D0))/D ! tau**3 * f'''/Tc
...
!
        ...
        do i = iFirst, iLast
            ...
            if ((cSolnPhaseType(iSolnPhaseIndex) == 'RKMPM').OR. &
                (cSolnPhaseType(iSolnPhaseIndex) == 'QKTOM')) then
                dMagGibbsEnergy(i) = dTempG
                dMagEnthalpy(i) = dTempH
                dMagEntropy(i) = dTempS
                dMagHeatCapacity(i) = dTempCp
            else if ((cSolnPhaseType(iSolnPhaseIndex) == 'SUBLM').OR.(cSolnPhaseType(iSolnPhaseIndex) == 'SUBOM')) then
                m = i - iFirst + 1
...
!
                        ...
                        if (k == l)  dPreFactor = dPreFactor + 1D0 / DMAX1(dSiteFraction(iChargedPhaseID,s,k), 1D-75)
                    end do
!
...
!
                LOOP_Param_Species: do i = iFirst, iLast
                    ...
                    dTemp = 1D0 - DFLOAT(n + iMagneticParam(iParam,n+2))
!
...
!
                            ...
                            dTemp = dTemp + 1D0 / DMAX1(dSiteFraction(iChargedPhaseID,s,c), 1D-75)
!
