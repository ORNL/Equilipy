...
!
    ...
    real(8) :: dExponent, dExponentMax, dWeightSum
    logical :: lPhasePass
!
!
...
!
        case ('QKTO','QKTOM','RKMP','RKMPM','SUBL','SUBLM','SUBOM','SUBG','SUBQ')
!
...
!
        case default
!
            ! The default case assumes an ideal solution phase.  The minimum
            ! against the current elemental-potential plane is available in
            ! closed form:
            !
            !     x_i = exp(mu_i^plane - g_i^0) / sum_k exp(mu_k^plane - g_k^0)
            !
            ! where all terms are dimensionless.  Use log-sum-exp so ideal gas
            ! candidates do not overflow, underflow, or retain the old clipped
            ! unnormalized weights.
            dExponentMax = -HUGE(1D0)
            do i = m, n
                dTemp = 0D0
                do j = 1, nElements
                ...
                dExponent       = dTemp - dStdGibbsEnergy(i)
                dMolFraction(i) = dExponent
                dExponentMax    = DMAX1(dExponentMax, dExponent)
            end do
!
            dWeightSum = 0D0
            do i = m, n
                dMolFraction(i) = DEXP(DMAX1(-745D0, dMolFraction(i) - dExponentMax))
                dMolFraction(i) = DMAX1(dMolFraction(i), 1D-300)
                dWeightSum = dWeightSum + dMolFraction(i)
            end do
!
            if (dWeightSum > 0D0) then
                dMolFraction(m:n) = dMolFraction(m:n) / dWeightSum
                dDrivingForceTemp = -(dExponentMax + DLOG(dWeightSum))
            else
                dMolFraction(m:n) = 1D0 / DFLOAT(n - m + 1)
