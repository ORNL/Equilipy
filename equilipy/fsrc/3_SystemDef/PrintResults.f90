...
subroutine PrintResultsSolnPhase
    ...
    call Qsort(-dTempVec, iTempVec, nSolnPhases)
    ...
    LOOP_SolnStable: do j = 1, nSolnPhases
        ...
        if ((cSolnPhaseType(l)) == 'SUBG' .OR. (cSolnPhaseType(l) == 'SUBQ')) then
            call CalculateCompositionSUBG(l,dMolesPairs,.TRUE.)
        end if
        ...
        if ((cSolnPhaseType(l) == 'SUBLM') .OR. (cSolnPhaseType(l) == 'SUBOM') .OR. &
            (cSolnPhaseType(l) == 'RKMPM')) then
            Tcritical = 0D0
            B = 0D0
        ...
        select case (cSolnPhaseType(l))
            ...
            case ('IDMX', 'RKMP', 'RKMPM', 'QKTO', 'QKTOM')
                ...
                call Qsort(-dTempSpecies, iTempSpecies, k)
...
subroutine PrintResultsPureConPhase
    ...
    call Qsort(-dTempVec, iTempVec, nConPhases)
