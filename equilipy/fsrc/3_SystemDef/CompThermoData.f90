        ...
        if ((cSolnPhaseTypeCS(n) == 'SUBG') .OR. (cSolnPhaseTypeCS(n) == 'SUBQ')) then
            ...
            LOOP_SROPairs: do i = iFirst, iFirst - 1 + nPairsSROCS(iSublPhaseIndex,1)
                ...
                if (iSpeciesPass(iIndex) > 0) then
                    ...
                    m = 0
                    do k = 1, nElemOrComp
                        if ((iElementSystem(k) > 0).OR.(iElementSystem(k) == -1)) then
                            m = m + 1
            ...
            LOOP_nSUBGQCS: do i = nSpeciesPhaseCS(n) + 1, nSpeciesPhaseCS(n+1)
                ...
                call SetLevelingSpeciesTotalAtoms(j, n, i)
            ...
            LOOP_nSpeciesCS: do i = nSpeciesPhaseCS(n) + 1, nSpeciesPhaseCS(n+1)
                ...
                if (iPhaseCS(i) == 1) then
                    ...
                    if (cSolnPhaseTypeCS(1) == 'IDMX') then
                        ...
                        dPartialEntropy(j) = dPartialEntropy(j) - (0.10945D0 / dIdealConstant)
                    end if
                end if
...
!
                ...
                else if ((cSolnPhaseTypeCS(i) == 'SUBLM').OR.(cSolnPhaseTypeCS(i) == 'SUBOM')) then
...
!
        ...
        if (dLevelingSpeciesTotalAtoms(i) <= 0D0) then
            dLevelingSpeciesTotalAtoms(i) = dSpeciesTotalAtoms(i)
        end if
        if (dLevelingSpeciesTotalAtoms(i) == 0D0) then
            dLevelingSpeciesTotalAtoms(i) = 0.00001D0
        end if
        if (dLevelingSpeciesFormulaAtoms(i) <= 0D0) then
            dLevelingSpeciesFormulaAtoms(i) = dLevelingSpeciesTotalAtoms(i)
        end if
        if (dLevelingSpeciesFormulaAtoms(i) == 0D0) then
            dLevelingSpeciesFormulaAtoms(i) = 0.00001D0
        end if

        dTemp = 1D0 / dLevelingSpeciesFormulaAtoms(i)
        dLevelingChemicalPotential(i) = dChemicalPotential(i) * dTemp + dMagGibbsEnergy(i)
        dTemp = 1D0 / dLevelingSpeciesTotalAtoms(i)
        do j = 1, nElements
            dLevelingCompositionSpecies(i,j) = dStoichSpecies(i,j) * dTemp
        end do
...
!
    ...
    LOOP_EID_Phase: do i = 1, nSolnPhasesSys
        ...
        if ((cSolnPhaseType(i) == 'SUBL').OR.(cSolnPhaseType(i) == 'SUBLM').OR. &
            (cSolnPhaseType(i) == 'SUBOM')) then
