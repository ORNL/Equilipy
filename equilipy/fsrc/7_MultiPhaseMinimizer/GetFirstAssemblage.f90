...
!
    iAtomFractionSpecies = INT(dLevelingCompositionSpecies)
...
!
    do i = 1,nSpeciesLevel
        dTempArr(i,1:nElements) = dLevelingChemicalPotential(i) * DFLOAT(iAtomFractionSpecies(i,1:nElements))
    end do
!
...
!
    do j = 1,nElements
        ...
        if ((dLevelingChemicalPotential(iAssemblage(j))-dLevelingChemicalPotential(iAssemblageTemp(j)))>dTolerance(5)) then
            iAssemblage(j)=iAssemblageTemp(j)
        end if
        do i = 1,nElements
            A(i,j) = dLevelingCompositionSpecies(iAssemblage(i),j)
        end do
        dElementPotential(j) = dLevelingChemicalPotential(iAssemblage(j)) !SY: dLeveling ->dElementalPotential
    end do
    INFO = 0
    IPIV = 0
...
!
    do i = 1, nElements
        do j = 1, nElements
            A(j,i) = dStoichSpeciesLevel(iAssemblage(i),j)
        end do
...
!
    ...
    do i = 1, nElements
        ...
        if(j==0) then
            ...
            if(iAssemblage(i)<=nSpecies) then
                dMolFractionGEM(i,m:n) = 0D0
                dMolFractionGEM(i,iAssemblage(i)) = 1D0
            else
                call CopyLevelCandidateMolFraction(iAssemblage(i), i, j, lCandidateFound)
