...
!
    ...
    logical                                :: lPhasePass, lCandidateFound
!
!
...
!
    ...
    LOOP_NewAssemblage: do k = 1, nElements
        ...
        dAtomFractionSpeciesGEM(k,:) = dLevelingCompositionSpecies(iNewPhase,:)
        ...
        if ((INFO==0).AND.(dMinValuedMolesPhase(k) >=-1D-10)) exit LOOP_NewAssemblage
        ...
        if (k<nElements) then 
            ...
            ! Always initialize the replacement index before the tolerance check.
            ! Otherwise the fallback below can use a stale DO-loop value for i and
            ! corrupt iAssemblage.
            ...
            dChemicalPotentialGEM(i)     = dLevelingChemicalPotential(iNewPhase)
            dStoichSpeciesGEM(i,:)       = dStoichSpeciesLevel(iNewPhase,:)
            dAtomFractionSpeciesGEM(i,:) = dLevelingCompositionSpecies(iNewPhase,:)
