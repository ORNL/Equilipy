...
subroutine CheckConvergencePEA
    ...
    if (lDebugMode) print *, "Test 1 ", MINVAL(dMolesPhase)
    if (MINVAL(dMolesPhase) < 0D0) return
    

    ! TEST #2: Check that the Phase Rule has been satisfied:
    ! -----------------------------------------------------------------------------------
    ! if (lDebugMode) print *, "Test 2 "
    ! if (nSolnPhases + nConPhases > nElements - nChargedConstraints) call CorrectPhaseRule(lPhaseChange)
    ! if (lPhaseChange) return


    ! TEST #3: Check the relative errors of the mass balance equations:
    ! -----------------------------------------------------------------
    if (lDebugMode) print *, "Test 3 "
    LOOP_TEST3: do j = 1, nElements
    ...
    ! TEST #4: Check if there is any phase to add:
    ! -----------------------------------------------------------------------------------
    
    call CompDrivingForceAll
    if (lDebugMode) print *, "Test 4: MINVAL(dPhasePotential) ",MINVAL(dPhasePotential)
    if(MINVAL(dPhasePotential)<0D0) return

    ! TEST #5:CONVERGENCE TEST SHORTCUT
    ! -----------------------------------------------------------------------------------
    if (lDebugMode) print *, "Test Convergence ", dGEMFunctionNorm
    if (dGEMFunctionNorm < 1D-6) then
        lConverged = .TRUE.
        return
    else if((dGEMFunctionNorm < 1D-4).AND.(iterGlobal > 1000))  then
        lConverged = .TRUE.
        return
    else if ((dGEMFunctionNorm < 1D-3).AND.(iterGlobal > 2000)) then
        lConverged = .TRUE.
        return
    end if
    ...
    ! print*,'dGEMFunctionNorm',dGEMFunctionNorm,dTolerance(1)
    ...
    if (iMaxDrivingForce > 0) then
        if (lDebugMode) print *, "Test 6 ", dMaxDrivingForce, cSpeciesName(iMaxDrivingForce)
    else
        if (lDebugMode) print *, "Test 6 ", dMaxDrivingForce
    end if
    ! print*,'dMaxDrivingForce',dMaxDrivingForce
    if (dMaxDrivingForce < dTolerance(4)) return

    ! TEST #5: Check the residuals of chemical potential terms:
    ! ---------------------------------------------------------
    if (lDebugMode) print *, "Test 7 "
    LOOP_TEST7: do j = 1, nSolnPhases
    ...
    ! TEST #8: Check the residuals of site fractions on each sublattice of a CEF phase:
    ! ---------------------------------------------------------------------------------
    if (lDebugMode) print *, "Test 8 "
    ! Loop through solution phases that are stable:
    LOOP_TEST8: do j = 1, nSolnPhases
    ...
    ! TEST #9: Check if a solution phase should be added to the phase assemblage:
    ! ---------------------------------------------------------------------------
    ...
    LOOP_TEST9: do i = 1,nSolnPhasesSys
        ...
        if (lMiscibility(i) .EQV. .TRUE.) cycle LOOP_TEST9
    ...
    LOOP_TEST10: do i = 1, nSolnPhasesSys
        ...
        if (lMiscibility(i) .EQV. .TRUE.) then
            ...
            LOOP_DoubleCheckMG: do j = 1, nSolnPhases
                ...
                if ((cSolnPhaseName(k) == cSolnPhaseName(i)).AND.(lSolnPhases(k) .EQV. .FALSE.)) cycle LOOP_TEST10
...
end subroutine CheckConvergencePEA
