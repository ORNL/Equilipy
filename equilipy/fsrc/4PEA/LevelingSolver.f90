...
!
    ...
    !Consider taking this out as post process leveling
    ...
    allocate(dMolesPhaseLast(nElements),iAssemblageGEM(nElements))
...
!
    ...
    do i = 1, nElements
        j  = iPhaseLevel(iAssemblageGEM(i))
...
!
        if ((iPhaseLevel(iAssemblageGEM(i)) <= 0).AND.(nConPhases + nSolnPhases < nElements)) then
            nConPhases              = nConPhases + 1
            iAssemblage(nConPhases) = iAssemblageGEM(i)
            dMolesPhase(nConPhases) = dMolesPhaseLast(i)
        elseif ((iPhaseLevel(iAssemblageGEM(i)) > 0).AND.(nConPhases + nSolnPhases < nElements)) then
!
            do j = 1,nSolnPhases
                k = nElements - j + 1
                ! Ensure that this solution phase is not already stored:
                if (iAssemblage(k) == -iPhaseLevel(iAssemblageGEM(i))) cycle LOOP_AddPhase
            end do
!
            nSolnPhases = nSolnPhases + 1
!
            j = nElements - nSolnPhases + 1
            k = iPhaseLevel(iAssemblageGEM(i))
!
            iAssemblage(j) = -k
