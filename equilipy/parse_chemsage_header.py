"""Parse ChemSage database header sections."""
...
def ParseCSHeader():
    """Parse the ChemSage database header into module variables."""
    ...
    if var.nSolnPhasesSysCS == 1 and var.nSpeciesPhaseCS[0] == 0:
        var.nSolnPhasesSysCS = 0

    # Check the number of solution phases are under max number
    assert var.nSolnPhasesSysCS <= var.nSolnPhasesSysMax, (
        "Error: Number of solution phases exceeds the maximum number: "
        + str(var.nSolnPhasesSysMax)
    )
    ...
    var.iDisorderedPhaseCS = np.zeros(j, dtype=int)
    var.nPairsSROCS = np.zeros((j, 2), dtype=int)
    var.iPairIDCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSpeciesPhaseCS, 4), dtype=int
    )
    ...
    var.nConstituentSublatticeCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS), dtype=int
    )
    var.iConstituentSublatticeCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS, var.nMaxSpeciesPhaseCS), dtype=int
    )
    var.nSublatticeElementsCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS), dtype=int
    )
    var.iChemicalGroupCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS, var.nMaxSpeciesPhaseCS), dtype=int
    )

    # Memory allocation
    var.dAtomicMass = np.zeros(var.nElementsCS, dtype=float)
    var.dCoordinationNumberCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSpeciesPhaseCS, 4), dtype=float
    )
    var.dStoichSpeciesCS = np.zeros((var.nSpeciesCS, var.nElementsCS), dtype=float)
    var.dGibbsCoeffSpeciesTemp = np.zeros(
        (var.nGibbsCoeff, var.nSpeciesCS * var.nMaxGibbsEqs), dtype=float
    )
    ...
    var.dStoichSublatticeCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS), dtype=float
    )
    var.dZetaSpeciesCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSpeciesPhaseCS), dtype=float
    )
    var.dConstituentCoefficientsCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSpeciesPhaseCS, 5), dtype=float
    )
    var.dSublatticeChargeCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSublatticeCS, var.nMaxSpeciesPhaseCS),
        dtype=float,
    )
    var.dStoichPairsCS = np.zeros(
        (var.nSolnPhasesSysCS, var.nMaxSpeciesPhaseCS, var.nElementsCS), dtype=float
    )

    var.cSolnPhaseNameCS = ["{:<25}".format("")] * j
    var.cSolnPhaseTypeCS = ["{:<8}".format("")] * j
    var.cDisorderedPhaseNameCS = [""] * j
    ...
    for i in range(len(var.cElementNameCS)):
        if var.cElementNameCS[i][0] == "e":
            var.cElementNameCS[i] = "e- "
    ...
    assert sum(iGequation - [6, 1, 2, 3, 4, 5, 6]) == 0, (
        "Themperature dependent G is not implemented"
    )
    del var.DataBase[:14]
    return var.DataBase
