"""Parse QKTO solution model data blocks."""
...
def ParseCSDataBlockQKTO(i):
    """Parse QKTO excess parameters for one solution phase."""
    ...
    if "QKTOM" in var.cSolnPhaseTypeCS[i - 1]:
        ...
        while True:
            ...
            if var.iMagneticParamCS[var.nMagParamCS, 0] == 0:
                ...
                del var.DataBase[:4]
                var.dMagneticParamCS[var.nParamCS, :6] = np.asarray(
                    var.DataBase[:6], dtype=float
                )
                del var.DataBase[:6]
            elif var.iMagneticParamCS[var.nMagParamCS, 0] == 3:
                # Ternary mixing
                var.iMagneticParamCS[var.nMagParamCS, 1:7] = np.asarray(
                    var.DataBase[:6], dtype=int
                )
                del var.DataBase[:4]
                var.dMagneticParamCS[var.nParamCS, :6] = np.asarray(
                    var.DataBase[:6], dtype=float
    ...
    while True:
        ...
        if var.iRegularParamCS[var.nParamCS, 0] == 0:
            ...
            # Ternary mixing term
            var.iRegularParamCS[var.nParamCS, 1:7] = np.asarray(
                var.DataBase[:6], dtype=int
            )
            del var.DataBase[:6]
            var.dRegularParamCS[var.nParamCS, :6] = np.asarray(
