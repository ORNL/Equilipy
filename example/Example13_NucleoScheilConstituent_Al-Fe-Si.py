...
if __name__ == "__main__":
    ...
    res_scheil = res = eq.scheil_cooling(
        liquid_phase_name, database, condition, delta_T=0.1, unit=["C", "atm", "g"]
    ).scheil_constituents
    ...
    df = pl.concat(
        [pl.DataFrame(res_scheil), pl.DataFrame(res_nucleoscheil)], how="diagonal"
    )
    ...
    for ax in [ax1, ax2]:
        ...
        for i in range(num_packages):
            ...
            bars = ax.bar(
                ...
                performance_data[:, i],
                width=bar_width,
                label=packages[i] if ax == ax1 else "",
                edgecolor="k",
                color=colori[i],
            )

            # Add labels on the subplot that contains each bar.
            for _j, bar in enumerate(bars):
                ...
                if (ax == ax1 and height >= 30) or (ax == ax2 and height < 30):
                    ...
                    ax.text(
                        bar.get_x() + bar.get_width() / 2.0 + 0.03,
                        label_y,
                        f"{height:.0f}%",
                        ha="center",
                        va="bottom",
    ...
    # Save and display
    fig.savefig(
        os.path.join(fpath, "ScheilConstituents.svg"),
        transparent=False,
        bbox_inches="tight",
    )
    plt.show()
