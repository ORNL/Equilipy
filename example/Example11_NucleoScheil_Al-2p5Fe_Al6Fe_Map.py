...
def process_data(df: pd.DataFrame, phase_map: dict) -> pd.DataFrame:
    """
    Load phase data and create a configuration column from active phases.

    Args:
        df: The input phase data.
    ...
    def get_config_label(row_of_bools):
        """Generate a sorted, combined label for active phases in a row."""
        # Select the phase names where the row value is True
        active_labels = [
            phase_map[col] for col, is_active in row_of_bools.items() if is_active
        ]
...
def plot_phase_map(df: pd.DataFrame, description_map: dict):
    """
    Create and format a scatter plot of the phase configurations.

    Args:
        df: The processed DataFrame containing the data to plot.
        description_map: A map from short labels back to full phase names.
    """
    # 1. Set up the plot
    ...
    al13fe4_label = (
        r"$\boldsymbol{\Delta}$ T$^{\boldsymbol{*}}_"
        r"{\boldsymbol{Al}_{\boldsymbol{13}}\boldsymbol{Fe}_{\boldsymbol{4}}}$ [K]"
    )
    al6fe_label = (
        r"$\boldsymbol{\Delta}$ T$^{\boldsymbol{*}}_"
        r"{\boldsymbol{Al}_{\boldsymbol{6}}\boldsymbol{Fe}}$ [K]"
    )
    ax.set_xlabel(al13fe4_label, fontdict=axfont)
    ax.set_ylabel(al6fe_label, fontdict=axfont)
    ...
    # descriptive_labels = [
    #     f"{code} : {description_map.get(code, 'N/A')}" for code in labels
    # ]
...
if __name__ == "__main__":
    ...
    res = eq.nucleoscheil_constituent_batch(
        liquid_phase_name,
        database,
        condition,
        critical_undercooling,
    ...
    ucs = pl.DataFrame(
        {
            f"dT_{key}": critical_undercooling[key]
            for key in critical_undercooling.keys()
        }
    )
